...
Session(app)
# mydb=mysql.connector.connect(host='localhost',user='root',password='manasa@04',db='ecommy')
...
with mysql.connector.connect(host=host,port=port,user=user,password=password,db=db) as conn:
    ...
    cursor.execute("CREATE TABLE user (user_name varchar(255) NOT NULL,mobile_no bigint NOT NULL,email varchar(255) NOT NULL,address text NOT NULL,password varbinary(255) DEFAULT NULL,PRIMARY KEY (email),UNIQUE KEY mobile_no (mobile_no))")
    cursor.execute("additems | CREATE TABLE additems (itemid binary(16) NOT NULL,item_name longtext NOT NULL,dis longtext NOT NULL,qyt int DEFAULT NULL,category enum(electronics,home,fashion,grocery) DEFAULT NULL,price int DEFAULT NULL,added_by varchar(255) DEFAULT NULL,img_id varchar(255) DEFAULT NULL,PRIMARY KEY (itemid),KEY added_by (added_by),CONSTRAINT additems_ibfk_1 FOREIGN KEY (added_by) REFERENCES vendor (email) ON DELETE CASCADE ON UPDATE CASCADE)")
    cursor.execute("CREATE TABLE orders (ordid binary(16) NOT NULL,item_name varchar(255) DEFAULT NULL,qty int DEFAULT NULL,total_price decimal(20,4) DEFAULT NULL,item_id binary(16) NOT NULL,user varchar(255) DEFAULT NULL,category varchar(255) DEFAULT NULL,imgid varchar(255) DEFAULT NULL,dis text,PRIMARY KEY (ordid),KEY item_id (item_id),KEY user (user),CONSTRAINT orders_ibfk_1 FOREIGN KEY (item_id) REFERENCES additems (itemid) ON DELETE CASCADE,CONSTRAINT orders_ibfk_2 FOREIGN KEY (user) REFERENCES user (email) ON DELETE SET NULL ON UPDATE CASCADE)")
            ...
            if uotp == data['otp']:
                ...
                # generating the salt
                 ...
                 # Hashing the password
                    ...
                    # Hashing the password
                    cursor = mydb.cursor(buffered=True)
                    cursor.execute('INSERT INTO user (email, user_name, mobile_no, address, password) VALUES (%s, %s, %s, %s, %s)',(data['email'],data['user_name'], data['mobile_no'], data['address'], hash))
                    mydb.commit()
                    cursor.close()
                    flash('Registration successfully done.')
...
def success(itemid,itemname,q,total,category,imgid,dis):
    user=session.get('user')
    cursor=mydb.cursor(buffered=True)
    cursor.execute('insert into orders(ordid,item_id,item_name,qty,total_price,user,category,imgid,dis) values(uuid_to_bin(uuid()),uuid_to_bin(%s),%s,%s,%s,%s,%s,%s,%s)',[itemid,itemname,q,total,user,category,imgid,dis])
    mydb.commit()
...
def orders():
    if session.get('user'):
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select bin_to_uuid(ordid),bin_to_uuid(item_id),item_name,qty,total_price,user,category,imgid,dis from orders where user=%s',[session.get('user')])
        count=cursor.fetchall()
...
def invoice(ordid):
    cursor=mydb.cursor(buffered=True)
    cursor.execute('select user_name,mobile_no,address,u.email,bin_to_uuid(ordid),bin_to_uuid(item_id),item_name,qty,total_price,category,imgid,dis from user u join orders o on u.email=o.user where ordid=uuid_to_bin(%s)',[ordid])
    count=cursor.fetchone()
    if count:
